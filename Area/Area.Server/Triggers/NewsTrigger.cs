...
{
    public class NewsTrigger : ITrigger
    {
        ...
        public NewsTrigger(ReactionEnum _reaction, string _params)
        {
            Reaction = _reaction;
            Params = _params;
            running = false;
        ...
        private void Work()
        {
            string data = NewsService.GetNewsByTag(Params);
            string title = "News Update " + DateTime.Now.ToString();
        ...
        public override void Start()
        {
            if (running)
                return;
            Logger.Debug("News trigger started.");
