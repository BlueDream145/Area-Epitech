using Area.MobileClient.Handlers;
...
namespace Area.MobileClient.Managers
{
            ...
            foreach (Type t in ass.GetTypes())
            {
                ...
                foreach(MethodInfo method in methods)
                {
                    if (method.Name.ToLower().StartsWith("handle"))
                    {
                        ...
                        if (param.Length != 2 || param[1].ParameterType.BaseType != typeof(NetworkMessage))
                            continue;
                        Cache.Add(param[1].ParameterType, new KeyValuePair<Type, MethodInfo> (t, method));
                    }
                }
            }
